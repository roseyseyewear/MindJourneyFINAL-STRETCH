...
export default function FuturisticChatInterface({
  ...
  useEffect(() => {
    const initialMessage: ChatMessage = {
      ...
      sender: 'lab',
      type: 'text',
      content: DELTA_COPY.chatInterface.greeting.hello,
      timestamp: new Date(),
    };
    ...
    setTimeout(() => {
      ...
      const hypothesisMessage: ChatMessage = {
        ...
        sender: 'lab',
        type: 'text',
        content: DELTA_COPY.chatInterface.greeting.hypothesis_prompt,
        timestamp: new Date(),
      };
  ...
  const handleTextResponse = async (questionId: string, value: string) => {
    ...
    try {
      ...
      setTimeout(() => {
        addLabMessage(DELTA_COPY.chatInterface.acknowledgments.text_received);
        moveToNextQuestion();
      }, 1000);
  ...
  const handleFileUpload = async (questionId: string, type: 'photo' | 'video', file: File) => {
    ...
    try {
      ...
      // Add acknowledgment from The Lab
      setTimeout(() => {
        const acknowledgment = type === 'photo' ? 
          DELTA_COPY.chatInterface.acknowledgments.photo_received : 
          DELTA_COPY.chatInterface.acknowledgments.video_received;
        addLabMessage(acknowledgment);
        moveToNextQuestion();
      }, 1000);
  ...
  const sendVoiceNote = async (questionId: string) => {
    ...
    try {
      ...
      // Add acknowledgment from The Lab
      setTimeout(() => {
        addLabMessage(DELTA_COPY.chatInterface.acknowledgments.audio_received);
        moveToNextQuestion();
      }, 1000);
  ...
  const moveToNextQuestion = () => {
    ...
    if (nextIndex < questions.length) {
      ...
      setTimeout(() => {
        addLabMessage(DELTA_COPY.chatInterface.transitions.to_contact_collection);
        askForContactInfo();
      }, 1500);
    }
  };
  ...
  const askForContactInfo = () => {
    setTimeout(() => {
      addLabMessage(DELTA_COPY.chatInterface.transitions.to_name_request);
      setAwaitingResponse(true);
  ...
  const handleContactResponse = async (type: 'name' | 'email', value: string) => {
    if (type === 'name') {
      ...
      setTimeout(() => {
        addLabMessage(DELTA_COPY.chatInterface.transitions.to_email_request);
        setAwaitingResponse(true);
        setMessages(prev => [...prev.slice(0, -1), {
          ...prev[prev.length - 1],
      ...
      try {
        ...
        setTimeout(() => {
          addLabMessage(DELTA_COPY.chatInterface.transitions.completion);
                ...
                <div className="flex items-center rounded-full p-2" style={{ backgroundColor: 'rgba(238, 238, 238, 0.2)' }}>
                    ...
                    {!recordedBlob && (
                      <Input
                        ...
                        placeholder={DELTA_COPY.chatInterface.placeholders.hypothesis}
